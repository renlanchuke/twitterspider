...
var dataArray = common.getDateArray(queryOptions.since, queryOptions.until);
var lpara = '&reset_error_state=false';
function gettwitterByday(index) {
    if (index >= dataArray.length - 1) {

        logger.log("获取用户ID结束")
        return;
    }
    var currQueryOptions = queryOptions;
    currQueryOptions.since = dataArray[index];
    currQueryOptions.until = dataArray[index + 1];
    ...
    var query = 'https://twitter.com/i/search/timeline?vertical=default&' +
        handleQuery(currQueryOptions) + '&src=typd&' +
        'include_available_features=1&include_entities=1&last_note_ts=';

    //console.log(currQueryOptions);
    gettwitterid(last_note_ts, max_position, query),
        gettwitterByday(index + 1);
}

//递归求search twitter信息
function gettwitterid(lastNoteTS, maxPosition, query) {
    ...
    common.get(currURL, null, (err, data) => {
        ...
        //请求下一条记录
        gettwitterid(currLastNoteTS, currMaxPosition, query);
    });
}
